<template>
  <div class="orders marketRoader" style="padding: 15px;">
    <h1>{{ $t('contract.contract_order') }}</h1>
    <!-- 当前委托/历史委托 -->
          ...
          <div v-if="orderIndex==0">
            <el-table :data="historyEntrust">
              ...
              <el-table-column :label="$t('contract.kcj')">
                <template slot-scope="scope">
                  <span>{{scope.row.openPrice|SubStringZreo(4)}}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('contract.pcj')">
                <template slot-scope="scope">
                  <span>{{scope.row.closePrice | SubStringZreo(4)}}</span>
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('message.leveragedDeals')">
                <template slot-scope="scope">
              ...
              <el-table-column :label="$t('contract.yk')">
                <template slot-scope="scope">
                  <span class="green_text" v-if="scope.row.profit > 0">{{scope.row.profit|SubStringZreo(4)}}</span>
                  <span class="red_text" v-else>{{scope.row.profit|SubStringZreo(4)}}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('contract.handNum')">
                <template slot-scope="scope">
                  <span>{{ scope.row.matchCount}}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('contract.fx')" fixed="right">
                <template slot-scope="scope">
                  <span class="red" v-show="scope.row.tradeType=='CLOSE_DOWN'">{{$t('contract.stip7')}}</span>
                  <span class="green" v-show="scope.row.tradeType=='CLOSE_UP'">{{$t('contract.stip6')}}</span>
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getHistoryEntrust(type) {
      ...
      this.$get("contract/contractOrder/getHistoryOrder", {
        member: this.$store.state.userId,
        pairsName: "",
        ...
        .then((res) => {
          // if (loading) loading.close();
          if (res && res.status == "SUCCEED") {
            this.historyEntrust = res.result.records
 || [];
            console.log(this.historyEntrust)
            if (this.isLogin) {
              this.timer1 = setTimeout((res) => {
