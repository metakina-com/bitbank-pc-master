    ...
    methods: {
        ...
        onMessage(data) {
            ...
            if (data.data && data.data.length) {
                ...
                //     console.error('不一样的交易对1：'  + this.pairsName +'!='+data.pairs)
                //     return
                // }
    ...
    //如果增量更新数据的时间大于缓存时间，而且缓存有数据 Allen
          ...
          //保留当前回调 allen  6.4
            ...
            if (true) {
                ...
                if (resolution < 60) {
                    ...
                    rangeStartDate = (rangeEndDate*1000  - (60*1000*10))/1000;
        ...
        //  this.cacheData[ticker] = [];
            ...
            if(this.interval==1){ //处理一分钟
                ...
                console.log(cacheDataMin1.length)
                if(cacheDataMin1.length>0){
                     if(cacheDataMin1[cacheDataMin1.length-1].time==this.lastTime){
                         onLoadedCallback(cacheDataMin1, { noData: false }); 
                     }else{
                         cacheDataMin1 = [];
