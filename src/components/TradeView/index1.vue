...
export default {
  ...
  methods: {
    ...
    setTime(interval) {
      ...
      //  console.error('发送指令前2时间戳：' +  new Date().getTime());
    },
    toClose1() {
      this.socket.toClose();
    },
    ...
    onMessage(data) {
      ...
      if (data.data && data.data.length) {
        ...
        //    console.error('解压后处理前时间戳：' +  new Date().getTime());

        let list = [];
        let dataarray = data.data;
        const ticker = `${this.symbol}-${this.interval}`;

        // console.log(this.interval + ':'  + dataarray.length)
        ...
        //    console.error('解压后处理后时间戳：' +  new Date().getTime());
        console.log(list)
        this.$hub.$emit("kdata", list);
        ...
        if (this.pairsName != data.pairs) {
          // console.error('不一样的交易对2：'  + this.pairsName +'!='+data.pairs)
          // return
        }
        ...
        //如果增量更新数据的时间大于缓存时间，而且缓存有数据 Allen
        ...
        //         // console.log("aaa:" + barsData.close)
        //     } else {
        //         let newdatacallback = _this.subBars; //回调新K线的方法
        //         newdatacallback.listener(barsData);
        //         this.$hub.$emit('fromRandomCurrentPrice', barsData.close);
        //         // console.log("bbb:" + barsData.close)
        ...
        // let radomTime = parseInt(Math.random() * 1000 + 4000);
    ...
    getBars(
      ...
      // rangeStartDate = rangeEndDate - (60*100);
      // console.error('时间戳：' +  new Date().getTime());
      ...
      if (true) {
        ...
        if (resolution < 60) {
          // if(resolution==1){
          //     rangeStartDate = rangeEndDate - (60*50);
          // }
      ...
      // console.error('发送指令时间戳：' +  new Date().getTime());

      // console.log(rangeStartDate *1000 ,rangeEndDate * 1000)

      // let cha = this.humandate(rangeStartDate *1000 ,rangeEndDate * 1000);
      // console.error(cha)

      let ticker = `${this.symbol}-${this.interval}`;
      //  this.cacheData[ticker] = [];

      this.$hub.$on("kdata", list => {
        //    console.error('马上渲染时间戳：' +  new Date().getTime());

        onLoadedCallback(list, { noData: false });
      });

      // const self = this;
      // if(this.interval==1){ //处理一分钟
      //     let cacheDataMin1 = this.cacheDataMin1;
      //     // console.log(cacheDataMin1.data.length)
      //     if(cacheDataMin1.data.length>0){

      //         //  console.log(this.symbol)
      //         //  console.log(symbolInfo)

      //          if(cacheDataMin1.data[cacheDataMin1.data.length-1].time==this.lastTime
      //          && this.symbol == cacheDataMin1.name
      //          ){
      //              onLoadedCallback(cacheDataMin1.data, { noData: false });
      //          }else{
      //              cacheDataMin1 = {};
      ...
      //         }
      //         });
      //         onLoadedCallback(newBars, { noData: false });
      //     }else{
      //         this.getBarTimer = setTimeout(function () {
      //             self.getBars(
      //                 symbolInfo,
