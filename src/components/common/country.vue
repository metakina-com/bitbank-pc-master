<template>
  <div class="ddd" style="position: relative;width:100px">
    <el-select
      v-model="value"
      placeholder="请选择国家区号"
      @input="change($event)"
      ...
      @focus="optionS"
    >
      <!-- <el-option v-for="(item,index) in options" :key="item.value" :label="`+${item.n}${item.c}(${item.o})`" :value="index"> -->
      <el-option
      ...
      >
        ...
        <span style="float: right; color: #8492a6; font-size: 13px">{{ item.c }}({{item.o}})</span>
      </el-option>
    </el-select>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    optionS() {
      var selectOption = document.getElementsByClassName("el-select-dropdown");
      selectOption[0].style.width = "380px";
      selectOption[0].style.left = "15px";
    }
  }
};
</script>
...
/* input {
    ...
    border-bottom: 1px solid #F4F8FD !important;
