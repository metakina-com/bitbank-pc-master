...
// const wsURL = 'wss://i.FVEX.global/wss'; //线上
const wsURL = 'wss://api.ststoken.com/wss'; //线上
// const wsURL = 'ws://121.89.219.116:8755/wss'; //测试
            ...
            reader.onload = (evt) => {
                if (evt.target.readyState == FileReader.DONE) {
                    ...
                    try {
                        ...
                        try{
                            if(data.indexOf('volume')!=-1&&data.indexOf('data')!=-1){ // 正常数据 k线的
                                data = this.handerData(JSON.parse(data))
                            }else{ // 盘口数据
                                      // } catch {
                             // 和约 盘口数据  matchList
